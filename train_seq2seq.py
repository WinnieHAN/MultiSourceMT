...
def main():
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') #'cpu' if not torch.cuda.is_available() else 'cuda:0'
    ...
    for i in range(EPOCHS):
        ...
        for _, batch in enumerate(train_iter):
            ...
            # masks2 = masks2.long()
            dec_out = trg
            dec_inp = torch.cat((trg[:, 0:1], trg[:, 0:-1]), dim=1)  # maybe wrong
            # train_seq2seq
        ...
        if True:  # i%1 == 0:
            ...
            for _, batch in enumerate(dev_iter):  # for _ in range(1, num_batches + 1):  word, char, pos, heads, types, masks, lengths = conllx_data.get_batch_tensor(data_dev, batch_size, unk_replace=unk_replace)  # word:(32,50)  char:(32,50,35)
                ...
                bleus = []
                num_words = len(en_field.vocab.stoi)
