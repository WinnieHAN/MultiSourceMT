...
class Seq2seq_Model(nn.Module):
    def __init__(self, EMB=8, HID=64, DPr=0.5, vocab_size1=None, vocab_size2=None, vocab_size3=None, word_embedd=None, device=None):
        ...
        if self.bidirectional:
            self.cat1 = nn.Linear(self.HID * 2* 2, self.HID* 2)
            self.cat2 = nn.Linear(self.HID * 2* 2, self.HID* 2)
            self.cat3 = nn.Linear(self.HID * 2* 2, self.HID* 2)
            ...
            self.cat1 = nn.Linear(self.HID * 2, self.HID)
            self.cat2 = nn.Linear(self.HID * 2, self.HID)
            self.cat3 = nn.Linear(self.HID * 2, self.HID)
    ...
    def forward(self, inp1, inp2,
        ...
        inp2 = self.emb2(inp2)
        out_enc2, h2 = self.enc(inp2)  # (50, 30, 128) (2, 50, 64)
        out_enc2 = self.DP(out_enc2)
        if self.bidirectional:
            if self.isLSTM:
                h2 = (h2[0].view((self.num_layers, 2, inp2.shape[0], self.HID)).transpose(1, 2).contiguous().view((self.num_layers, inp2.shape[0], 2*self.HID)), h2[1].view((self.num_layers, 2, inp2.shape[0], self.HID)).transpose(1, 2).contiguous().view((self.num_layers, inp2.shape[0], 2*self.HID)) ) # **h_n** of shape `(num_layers * num_directions, batch, hidden_size)`
            else:
                h2 = h2.view((self.num_layers, 2, inp2.shape[0], self.HID)).transpose(1, 2).contiguous().view((self.num_layers, inp2.shape[0], 2*self.HID))
        else:
            if self.isLSTM:
                h2 = (h2[0].view((self.num_layers, 1, inp2.shape[0], self.HID)).transpose(1, 2).contiguous().view((self.num_layers, inp2.shape[0], self.HID)), h2[1].view((self.num_layers, 1, inp2.shape[0], self.HID)).transpose(1, 2).contiguous().view((self.num_layers, inp2.shape[0], self.HID)) ) # **h_n** of shape `(num_layers * num_directions, batch, hidden_size)`
            else:
                h2 = h2.view((self.num_layers, 1, inp2.shape[0], self.HID)).transpose(1, 2).contiguous().view((self.num_layers, inp2.shape[0], self.HID))
