...
def main():
    ...
    device = torch.device('cuda:2') #torch.device('cuda' if torch.cuda.is_available() else 'cpu') #'cpu' if not torch.cuda.is_available() else 'cuda:0'
    ...
    print('time: ', time.asctime( time.localtime(time.time()) ))
    with open('seq2seq_test_data.pickle', 'wb') as f:
        pickle.dump(seq2seq_dev_data, f)
    with open('seq2seq_train_data.pickle', 'wb') as f:
        pickle.dump(seq2seq_train_data, f)
    # en_train_data = datasets.TranslationDataset(path='wmt14_3/train', exts=('.en', '.en'), fields=(en_field, en_field))
    # print('end en data add-----')
    # print('time: ', time.asctime( time.localtime(time.time()) ))
