...
def main():
    ...
    print('en_field.vocab size: ',len(en_field.vocab.itos))
    print('de_field.vocab size: ',len(de_field.vocab.itos))
    print('fr_field.vocab size: ',len(fr_field.vocab.itos))

    # # en
    # num = 20000
    # count = en_field.vocab.freqs
    # items = count.most_common(num)
    # itos = ['<unk>', '<pad>']+[i[0] for i in items]
    # num = num + 2
    # print('len(itos): ', len(itos))
    # for vocab_idx in range(len(itos)):
    #     en_field.vocab.stoi[itos[vocab_idx]] = vocab_idx
    # for i in count.keys():
    #     if (not i in itos) and (i in en_field.vocab.stoi):
    #         en_field.vocab.stoi.pop(i)
    # en_field.vocab.itos = itos
    # with open(str(num)+'_vocab_en.pickle', 'wb') as f:
    #     pickle.dump(en_field.vocab, f)

    # #     de
    # num = 20000
    # count = de_field.vocab.freqs
    # items = count.most_common(num)
    # itos = ['<unk>', '<pad>']+[i[0] for i in items]
    # num = num + 2
    # print('len(itos): ', len(itos))
    # for vocab_idx in range(len(itos)):
    #     de_field.vocab.stoi[itos[vocab_idx]] = vocab_idx
    # for i in count.keys():
    #     if (not i in itos) and (i in de_field.vocab.stoi):
    #         de_field.vocab.stoi.pop(i)
    # de_field.vocab.itos = itos
    # with open(str(num)+'_vocab_de.pickle', 'wb') as f:
    #     pickle.dump(de_field.vocab, f)

    # #     fr
    # num = 20000
    # count = fr_field.vocab.freqs
    # items = count.most_common(num)
    # itos = ['<unk>', '<pad>']+[i[0] for i in items]
    # num = num + 2
    # print('len(itos): ', len(itos))
    # for vocab_idx in range(len(itos)):
    #     fr_field.vocab.stoi[itos[vocab_idx]] = vocab_idx
    # for i in count.keys():
    #     if (not i in itos) and (i in fr_field.vocab.stoi):
    #         fr_field.vocab.stoi.pop(i)
    # fr_field.vocab.itos = itos
    # with open(str(num)+'_vocab_fr.pickle', 'wb') as f:
